const fs = require("fs");
const { getGif } = require("./GifPicker");
const Teams = JSON.parse(
  fs.readFileSync("./data/product-teams.json", { encoding: "utf8" })
);
...
const sendSuccessMessage = async (client, data = {}) => {
  ...
  const blocks = finalBlockBuilder({
    team: subscriber,
    repoName: data.repo,
    releaseNum: data.revision,
    releaseURL: data.githubLink,
    image: gif,
    altText: `${data.subscriber} gif`,
  });
